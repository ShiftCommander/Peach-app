...
Chromium icon URLs are versioned in `manifest.json` whenever the launcher artwork changes. Chrome 144+ treats unchanged icon URLs as immutable for installed web apps, so changing the PNG bytes alone is not sufficient to refresh an existing Android installation.
